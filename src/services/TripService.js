...
const createTrip = (newTrip) => {
    return new Promise(async (resolve, reject) => {
        try {
            let { time, dayStart,price, numberOfBooked, numberOfAvailable, departure, destination, car } = newTrip.trip
            ...
            let create = await Trip.create({
                time, dayStart, numberOfBooked, numberOfAvailable, departure, destination, car,price
            })
...
const getAllTrip = (limit, page, sort, filter) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let getTrips = await Trip.find().sort({dayStart: -1}).populate('time').populate('departure').populate('destination').populate('car').
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let findTrips = await Trip.find({
                ...
                destination,
                dayStart: day
            }).populate('time').populate('departure').populate('destination').populate('car')
